...
const sqlParts = [
  'ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS cpf TEXT;',
  migrationSQL.split('-- Create function to update user profile')[1].split('-- Create function to update user password by user_id')[0],
  migrationSQL.split('-- Create function to update user password by user_id')[1]
];
...
async function applyMigration() {
  ...
  try {
    ...
    for (let i = 0; i < sqlParts.length; i++) {
      console.log(`\n📝 Executando parte ${i + 1}/${sqlParts.length}...`);
      const result = await executeSQL(sqlParts[i]);
      console.log(`✅ Parte ${i + 1} executada com status: ${result.status}`);
    }
    
    console.log('\n🎉 Migração aplicada com sucesso!');
    console.log('🔄 Recarregue a página e teste as funcionalidades.');
